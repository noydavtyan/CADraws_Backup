...
const fileName = year + "-" + month + "-" + date + "_" + hours + "-" + minutes + "-" + seconds;
...
const fetchRows = () => {
    ...
    .then((response) => {
        ...
        worksheet.cell(i,1).string("Company");
        worksheet.cell(i,2).string("Description");
        worksheet.cell(i,3).string("Order Date");
        worksheet.cell(i,4).string("Delivery Date");
        worksheet.cell(i,5).string("Worker Price");
        worksheet.cell(i,6).string("Total Price");
        worksheet.cell(i,7).string("Payed");
        worksheet.cell(i,8).string("Received");
        response.data.forEach(model => {
            i++;
            worksheet.cell(i,1).string(model.id);
